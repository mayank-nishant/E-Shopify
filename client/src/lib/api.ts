...
function getErrorMsg(error: unknown) {
  if (axios.isAxiosError(error)) {
    return error.response?.data?.errors?.[0]?.message || error.message || "The request could not be completed.";
  }
...
export async function apiGet<T>(url: string, config?: AxiosRequestConfig) {
  try {
    ...
    if (response.data.status === "error" || !response.data.data) {
      throw new Error(response.data.errors?.[0]?.message || "The request could not be completed.");
    }
...
export async function apiPost<TResponse, TBody = unknown>(url: string, body?: TBody, config?: AxiosRequestConfig) {
  try {
...
export async function apiPut<TResponse, TBody = unknown>(url: string, body?: TBody, config?: AxiosRequestConfig) {
  try {
...
export async function apiPatch<TResponse, TBody = unknown>(url: string, body?: TBody, config?: AxiosRequestConfig) {
  try {
