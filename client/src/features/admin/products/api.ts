...
export async function updateAdminCategory(categoryId: string, body: UpdateCategoryBody) {
  return apiPut<Category, UpdateCategoryBody>(`/admin/categories/${categoryId}`, body);
}

// products
export async function getAdminProducts(search?: string) {
  const query = search?.trim() ? `/admin/products?search=${encodeURIComponent(search.trim())}` : `/admin/products`;
...
function buildProductFormData(body: CreateProductBody | UpdateProductBody, files: File[]) {
...
export async function createAdminProduct(body: CreateProductBody, files: File[]) {
  const formData = buildProductFormData(body, files);

  return apiPost<Product, FormData>("/admin/products", formData);
}

export async function updateAdminProduct(productId: string, body: UpdateProductBody, files: File[]) {
  const formData = buildProductFormData(body, files);
