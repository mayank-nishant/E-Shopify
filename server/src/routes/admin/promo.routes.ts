...
function parsePromoPayload(req: Request) {
  ...
  requireText(code, "Promo code is required");
    ...
    if (Number.isNaN(percentage) || percentage < 1 || percentage > 100) {
        throw new AppError(400, "Please enter a discount percentage between 1 and 100.");
    }

    if (!Number.isInteger(count) || count < 1) {
        throw new AppError(400, "Please enter a valid promo count of at least 1.");
    }

    if (Number.isNaN(minimumOrderValue) || minimumOrderValue < 1) {
        throw new AppError(400, "Please enter a minimum order value of 0 or greater.");
    }
  ...
  if (endsAt <= startsAt) {
    throw new AppError(400, "End time should be after the start time");
  }
